...
export const importFromExcel = async (req, res) => {
  try {
    ...
    // Tạo tài khoản user cho mỗi sinh viên
    for (let i = 0; i < dataStudent.length; i++) {
      const hashedPassword = await argon2.hash(dataStudent[i].msv.toString());
      ...
      const newUser = {
        username: dataStudent[i].msv,
        password: hashedPassword,
        role: "student",lop: dataStudent[i].lop // Đã sửa getItem từ trước
      };
      
      // Sửa dòng này
      const existingUser = await Users.findOne({ username: dataStudent[i].msv });
      if (existingUser) {
        console.log(`Username ${dataStudent[i].msv} already exists, skipping...`);
        continue;
      }
    ...
    if (error.code === 'ER_DUP_ENTRY') {
      return res.status(400).json({ message: `Duplicate entry: ${error.sqlMessage}` });
    }
    console.log(error);
    res.status(500).json({ message: "Server error ~ importFromExcel" });
  }
