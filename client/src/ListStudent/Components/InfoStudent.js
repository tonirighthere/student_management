...
class InfoStudent extends Component {
  ...
  componentDidMount() {
    ...
    if (match && match.params && match.params.id) {
      ...
      CallApi(`student/${id}`, "GET", null).then((res) => {
        ...
        if (res && res.data && res.data.StudentDetail && res.data.StudentDetail.length > 0) {
          var data = res.data.StudentDetail[0];
          console.log("Student data received:", data);
          
          // Đảm bảo dữ liệu có cả id và _id
          if (data.id && !data._id) {
            data._id = data.id;
          ...
          console.error("Không có dữ liệu sinh viên hoặc định dạng không đúng:", res);
      ...
      console.error("Không tìm thấy ID sinh viên trong props:", match);
      alert("Không tìm thấy thông tin sinh viên. Vui lòng quay lại trang danh sách.");
    }
  }
  ...
  render() {
    ...
    // Kiểm tra nếu student là mảng rỗng hoặc không có dữ liệu
    if (!student || (Array.isArray(student) && student.length === 0) || Object.keys(student).length === 0) {
      return (
